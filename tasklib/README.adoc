...
WARNING: the following is obsolete - boot-gae now uses `.edn` files
for configuration.  see the `greetings-gae` test app for examples.
Try running `gae/servlets` with and without the `:keep` flag to see
